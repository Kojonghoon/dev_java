...
public class HintPart1 {
  ...
  public String account(String user) {
    ...
    my[1] = (imsi % 100) / 10;// 256%100 ->56/10 -> 5.6 -> 5만 남음
    my[2] = imsi % 10;// 256%10 ->몫은 25이고 나머지는 6이다.
    for (int i = 0; i < com.length; i++) {// 입력받은 숫자가 com배열에 존재하니? 네 - 볼, 아니오 - 제외
      for (int j = 0; j < com.length; j++) {// 만일 위에서 네 라면 스트라이크인지를 검사한다. com배열의 i자리와 my배열의 j자리가 같니
        // 숫자가 있어?
        if (com[i] == my[j]) {// 같은 숫자가 있는지만 비교하고 자리가 달라도 참이다.
          // 볼은 확보 되었다. 자리가 같은지 비교하자
          // 숫자의 자리도 같아?
          if (i == j) {// 43번에서 비교한 숫자의 자리가 사용자가 입력받은 자리까지도 같은거야? 네 - 스, 노 -볼
            strike++;
          } else {
            ball++;
          }
