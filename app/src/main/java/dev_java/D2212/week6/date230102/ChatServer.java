...
public class ChatServer implements Runnable {
  ...
  public void run() {
    ...
    try {
      ...
      // 대기상태 ~~~~~~ 시간이 가다가 chatClient에서 new Socket("서버IOP",3000);
      System.out.println("서버소켓 생성 완료 - 클라이언트 소켓 접속 기다리는중 ...");
      while (true) {
        // 아래 코드가 진행되는 시점은 언제지? - new Socket("192.168.10.77",)
        Socket client = server.accept();
        System.out.println("클라이언트측 : " + client.getInetAddress());// 접속한 클라이언트 정보 출력
        // 대기상태 풀림
        // 금융권주로 사용 - 보안강화 - 직렬화 기법 - 마샬링, 언마샬링 구간
