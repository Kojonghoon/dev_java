...
public class MainThread1_1 {// 나는 스레드 클래스가 아님
  // 단 메인스레드는 갖고 있음
  // 18-19-20(main start)-21(인스턴스화 - 아직 run호출안됨) - 24(implements 라서 필요함 상속이면 필요없음)
  //27(start() ->run():즉시 실행되지 않고 대기실에서 순서를 따짐 - 그래서 yiled, join)-48-49-50-53-55-59-60

  public static void main(String[] args) {
    System.out.println("====================================");
    System.out.println("main start");
    Runnable r = new MyRunnableOne();// 인테페이스 = 구현체클래스(override: run - 추상메소드) - 다형성
    // 코드를 조금만 수정(1달전에 코딩 - 오타 )하고 유지보수가 가능함
    // 많이 손대면 손댈수록 의존관계로 인해 다른 에러를 유발시킴
    Thread myThread = new Thread(r);
    // 첫째. 스레드가 아니어서 에러이고 둘째 static안에서는 this를 사용 불가함
    // Thread myThread = new Thread(this);//this 넌 누구니? MainThread1_1 난 스레드가 아니에요
    myThread.start();// 디폴트 가중치는 5이다.
    // Thread himThread = new MyRunnableTwo();// 나 자신이 스레드 일때 start 호출 하는 방법과 아닐때 방법
...
// 1) 상속
// 2) implements
// 공통점 - 둘다 run 메소드를 가질 수 있다.(재정의 할 수 있다.) - 그안에서 실행문을 처리하면 안전함
class MyRunnableTwo extends Thread {
  @Override
  public void run() {
    System.out.println("난 MyRunnableTwo run 호출");
  }
}
...
class MyRunnableOne implements Runnable {
  @Override
  public void run() {
    System.out.println("MyRunnableOne run메소드 호출 성공");
    first();
  }
...
/*
 * 클래스를 상속받으면 클래스 간에 결합도가 높아진다. ->재사용성이 떨어짐(오작동경험)
 * 기능적인 부분을 결합도를 낮추면서 테스트고 용이한 그러면서 오작동도 방지되는 방법
 * :인터페이스 중심의 코딩을 전개하는 방법 - 오버라이딩(Overriding중요)
 */