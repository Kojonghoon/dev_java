package dev_java.D2212.week5.date221227.network;
...
//상속을 받거나 implrements하면 부모클래스나 인터페이스가 정의하고 있는 메소드를 
...
//POJO F/W설계 - 인터페이스 중심 코딩 전개 수업 -Spring boot(전자정부프레임뭑)기반 MVC패턴수업진행
//자바스크립트수업(ES5,6,7 소개)- NodeJS
//리액트수업(객체., 클래스, 리액트훅, 메소드. 파라미터, 리턴타입)
public class TalkServer2 extends JFrame implements /* Runnable, */ ActionListener {
  ...
  public static void main(String[] args) { // 메인스레드라고도 함 - entry point
    ...
    ts.init();// 나는 서버, 대기탐, 누구를 기다림 (사용자, 클라이언트, 고객,업무담당자)
    // 내안에 run메소드가 재정의(오버라이드) 되어 있으니까.
    // 아래 코드가 에러발생하는 것은 Runnerble을 implements에서 제거하였기 때문임
  ...
  public void init() {
    ...
    // 그래서 벡터 안에는 클라이언트를 관리하는 스레드를 추가해야 함.
    // 그 스레드는 메세지를 듣고(청취하고) 들은 내용을 그대로 클라이언트 측에 내보냄
    // 200#토마토#오늘스터디할까? StringTokenizer st = new StringTokenizer("","#");
    // st.nextToken(): String-200
    // st.nextToken(): String-토마토 -닉네임
    // st.nextToken(): String-오늘스터티할까? -메세지
    ...
    // 왜 이렇체 처리하나요? - 여러 스레드가 존재하고 경합이 벌어지므로 우선순위를 따져가며 호출함
    // 어떻게 호출하나요?
    ...
    try {
      ...
      while (!isStop) {// 언제 while문안으로 진입하지? -> new Socket(ip서버의, port);
        ...
        // String -VARCHAR2, CHAT, int-number(5):9999 double - number(7,2) 99999.99
        // 이종간의 언어에서 데이터를 공유(static) 목적으로 설계하는 디자인 패턴이름 - DTO
        // DeptVO - this 사용 - 전변에 대한 초기화 ValueObject패턴 일부 - 오라클과 자바연동 필요함
        // 이벤트 처리 시 이벤트처리를 담당하는 클래스를 가리키는 목적으로 this를 사용함
        // ActionListener구현할 때 - 버튼.addActionListener(this)
        // new XXX(this) - 클래스 분리, 나눌때, MVC패턴으로 구현할 때
        // 생성자 호출 시 파라미터로 들어오는 this는 현재 인스턴스화된 객체를 가리킴
        // 여기서 this는 TalkServer임
        // 아래는 객체를 생성하는 것과 동시에 생성자를 호출하는데 클래스 이름을 바꾸었다 그러니까 에러발생
...
/*
 * 채팅서버 구축하기
 * 클라이언트 측에서 접속하면 접속해온 정보를 서버측 화면에서 볼 수 있다.(JFrame을 상속했음 - 로그남김)
 * 자바는 단일 상속만 가능함 - 이러한 약점을 인터페이스로 지원함 - 그래서 다중인터페이스 구현체는 가능함
 * 스레드 구현방법
 * 1)Thread 상속하기
 * 2)Runnable 인터페이스를 implements 하기
