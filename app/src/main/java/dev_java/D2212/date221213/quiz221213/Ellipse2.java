...
// final - 변수앞에(상수) - 메소드 앞에(오버리이딩 불가) - 클래스 앞에(상속 안됨 - 자손못가짐)
public class Ellipse2 {
  // 선언부 -전변은 초기화를 생략가능함 - 왜냐하면 생성자가 해주니까....
  static double area;// 0.0
  final double PI = 3.14;

  // 자바에서는 같은 이름의 메소드를 중복 선언할 수 있음.
  // 단 파라미터의 갯수가 다르거나 또는 타입이 다르면 달라야 함.-메소드오버로딩이라함
  double 면적구하기() {
    return 0.0;
  }
  ...
  public static void main(String[] args) {
    ...
    // new 다음에 오는 ㅡ클래스가 생성됨 - 이 때 생성자 호출됨(마치 메소드처럼)
    ...
    int i1 = Integer.parseInt(user1);// 첫번째 반지름을 원시형 int로 저장함-곱셈해야 하니까
    int i2 = Integer.parseInt(user2);// 두번째 반지름을 원시형 int로 저장함-곱셈해야 하니까
    e2.면적구하기(i1, i2);
    System.out.println("타원형의 넓이는 " + Ellipse2.area + "입니다");// 공유, 하나 ,원본이 바뀐다.
  }
}
